	...
	script SafariTabGroupDecorator
		...
		*)
		on switchGroup(groupName)
			...
			if getGroupName() is equal to groupName then return
			...
			tell application "System Events" to tell process "Safari"
				if groupName is missing value then
					...
					set menuItems to menu items of my _getTabGroupMenuUI()
					set previousIsALabel to false
					repeat with nextMenuItem in menuItems
						set isLabel to not enabled of nextMenuItem
						if enabled of nextMenuItem and previousIsALabel then
							exit repeat
						end if
						set previousIsALabel to not enabled of nextMenuItem
					end repeat
					click nextMenuItem
		...
		(*
			This is the Group dropdown button when the sidebar is not visible.
		
			NOTE: This changes a lot for each macOS version update.
		*)
		on _getTabGroupPickerUI()
			...
			tell application "System Events" to tell process "Safari"
				...
				on error
					menu button 1 of group 1 of toolbar 1 of mainWindow
		...
		(*
			This is the menu 1 of the 	#_getTabGroupPickerUI()
		*)
		on _getTabGroupMenuUI()
			set mainWindow to getFirstZoomableWindow()
			if mainWindow is missing value then return missing value
			
			set tabGroupPickerUI to _getTabGroupPickerUI()
			
			tell application "System Events" to tell process "Safari"
				-- menu 1 of menu button 1 of menu button 1 of toolbar 1 of mainWindow
				
				-- entire contents of menu button 1 of toolbar 1 of mainWindow
				return menu 1 of tabGroupPickerUI
				-- --> {minimum value:missing value, orientation:missing value, position:{581, 168}, class:menu item, accessibility description:missing value, role description:missing value, focused:missing value, title:"Homepage", size:{258, 24}, help:missing value, entire contents:{}, enabled:true, maximum value:missing value, role:"AXMenuItem", value:missing value, subrole:missing value, selected:false, name:"Homepage", description:missing value}
			end tell
