(*
	...
	@Build:
		./scripts/build-lib.sh app-wrappers/Safari/26.5/safari
...
on spotCheck()
	...
	logger's infof("Standard window present: {}", sut's getFirstZoomableWindow() is not missing value)
...
on new()
	...
	set appWithFileDialog to appWithFileDialogLib's new("Safari")
	...
	script SafariInstance
		...
		on isProfilesActive()
			do shell script "
				PROFILE_DIR=~/Library/Containers/com.apple.Safari/Data/Library/Safari/Profiles
		...
		on getFirstZoomableWindow()
			...
			tell application "System Events" to tell process "Safari"
				try
					-- return first window whose (enabled of second button is true and description of second button is "zoom button")
					return first window whose (enabled of second button is true and description of second button is "full screen button")
				end try
			end tell
			missing value
		...
		*)
		...
		on normalizeProfileName(profileName)
			if isProfilesActive() and profileName is missing value then return "Personal"
			
			profileName
		end normalizeProfileName
		
		on _newSafariWindow(profileName)
			(* NOTE: Will be an empty string if profiles isn't active. *)
			set normalizedProfileName to normalizeProfileName(profileName)
			if running of application "Safari" is false then
				activate application "Safari"
				delay 1
				
			else if isProfilesActive() then
	...
	if decSafariTabGroup is not missing value then
		decSafariTabGroup's decorate(baseInstance)
	else
		baseInstance
	end if
end new
