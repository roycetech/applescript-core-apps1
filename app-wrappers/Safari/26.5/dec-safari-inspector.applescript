(*
	...
	WARNING:
		Broken on macOS 15.3.1. Apple likes to move things around without
		actually updating the Safari version.
	...
	@Build:
		./scripts/build-lib.sh app-wrappers/Safari/26.5/dec-safari-inspector

	@Created: Wed, Aug 12, 2026, at 10:00:29 AM
	@Last Modified: 2026-03-24 17:45:56

	@Change Logs:
		Wed, Aug 12, 2026, at 10:00:36 AM - To expand functionality to navigate the inspector tabs.
...
on decorate(safariInstance)
	...
	script SafariInspectorDecorator
		...
		*)
		...
		on toggleWebInspector()
			if isInspectorPanelActive() then
				closeWebInspector()
			else
				openWebInspector()
			end if
		end toggleWebInspector
