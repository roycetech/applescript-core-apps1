(*
	...
	@Build:
		./scripts/build-lib.sh app-wrappers/Safari/26.6/dec-safari-tab-finder2
...
on spotCheck()
	...
	if caseIndex is 2 then
		...
		logger's infof("sutName: {}", sutName)
		...
		logger's infof("sutSubstring: {}", sutSubstring)
		...
		logger's infof("sutProfile: {}", sutProfile)
		...
		on accept(safariTab)
			...
			tell application "System Events" to tell process "Safari"
				try
					...
					set foundSystemEventWindow to (first window whose value of attribute "AXIdentifier" of menu button 1 of toolbar 1 contains "profile=" & profileName) -- No more group on Tahoe.
					set systemEventWindowName to name of foundSystemEventWindow
					-- logger's debugf("systemEventWindowName: {}", systemEventWindowName)
				end try
			end tell
			tell application "Safari"
				set safariTabName to name of safariTab
				-- logger's debugf("safariTabName: {}", safariTabName)
				systemEventWindowName ends with safariTabName
			end tell
		end accept
	end script
		...
		on _findWindowTab(scriptObj, direction)
			...
			repeat with nextWindow in safariWindows
				...
				try
					...
					if tabsToIterate is not missing value then
