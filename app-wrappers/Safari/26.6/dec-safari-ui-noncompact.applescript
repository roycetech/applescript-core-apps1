(*
	...
	@Build:
		./scripts/build-lib.sh 'app-wrappers/Safari/26.6/dec-safari-ui-noncompact'

	@Created: Wednesday, September 20, 2023 at 10:13:11 AM
	@Last Modified: 2026-09-02 10:44:00
	
	@Change Logs:
		Wed, Sep 02, 2026, at 10:44:00 AM - Duplicated from 26.4 and added #triggerPageMenu
		Wed, Apr 15, 2026, at 03:21:45 PM - Duplicated from 26.3 and added #getHtmlScrollAreaUI
*)
...
property PROFILE_DEFAULT : "Personal"
	...
	script SafariUiNoncompactDecorator
		...
		*)
		...
		(*
			...
			NOTE: Assumes the the default profile settings.
		...
		*)
		on isDefaultGroup()
			...
			tell application "System Events" to tell process "Safari"
				try
					...
					return exists( first menu button of toolbar 1 of front window whose value of attribute "AXIdentifier" starts with "TabGroupPickerButton?profile=" & PROFILE_DEFAULT)
		...
		on isPlaying()
			...
			tell application "System Events" to tell process "Safari"
				try
					return exists (first button of last group of toolbar 1 of mainWindow whose description is "Mute This Tab" or description is "Unmute This Tab")
				end try
			end tell
			false
		end isPlaying
		...
		*)
		...
		(*
			Opens the Page Menu on the front window's selected tab.

			NOTE: App window must already be at the frontmost.
		*)
		on triggerPageMenu()
			...
			tell application "System Events" to tell process "Safari"
				...
				try
					set selectedTabRadio to first radio button of UI element 1 of group 3 of toolbar 1 of front window whose value is true
					click button "Page Menu" of selectedTabRadio
				end try
			end tell
		end triggerPageMenu
