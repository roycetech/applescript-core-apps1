(*
	...
	@Build:
		./scripts/build-lib.sh app-wrappers/Automator/2.10/automator
...
on spotCheck()
	...
	set sut to new()
	set automatorProcess to processLib's new(APP_NAME)
	...
	if caseIndex is 1 then
		...
		tell sut
			forceQuitApp()
			launchAndWaitReady()
			dock's clickApp(APP_NAME) -- Mitigate intermittent app not launching problem
...
on new()
	...
	set configSystem to configLib's new(CONFIG_TYPE_SYSTEM)
	...
	script AutomatorInstance
		...
		on selectApplicationType()
			...
			activate application APP_NAME -- try this first instead of below, made it similar to when creating voice command.
		...
		on addAppleScriptAction()
			...
			showSidebar()
			activate application APP_NAME
			tell application "System Events" to tell process APP_NAME to keystroke "Run AppleScript"
