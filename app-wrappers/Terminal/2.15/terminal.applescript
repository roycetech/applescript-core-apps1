(*
	...
	@Build:
		./scripts/build-lib.sh app-wrappers/Terminal/2.15/terminal

	@Change Logs
		Thu, Jul 30, 2026, at 08:21:00 AM - Added #getScrollPosition, #setScrollPosition
		Thu, Jul 30, 2026, at 08:13:35 AM - Added #scrollToBottom
*)
...
on new()
	...
	script TerminalInstance
		...
		*)
		...
		(*
			@returns scroll position 0 (top) to 1 (bottom), or missing value if unavailable.
		*)
		on getScrollPosition()
			...
			try
				tell application "System Events" to tell process "Terminal"
					return value of scroll bar 1 of scroll area 1 of splitter group 1 of front window
		...
		on setScrollPosition(scrollValue)
			if scrollValue is missing value then return
			if winUtil's hasWindow("Terminal") is false then return

			try
				tell application "System Events" to tell process "Terminal"
					set value of scroll bar 1 of scroll area 1 of splitter group 1 of front window to scrollValue
				end tell
			end try
