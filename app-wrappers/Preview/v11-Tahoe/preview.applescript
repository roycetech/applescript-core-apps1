(*
	...
	NOTE: The Preview didn't bother to increment the version number despite
	updating the UI layout of the app.
	...
	TODO: Use the base script with access to the file dialog.

	@Change Logs:
		Mon, Nov 17, 2025, at 11:58:39 AM - macOS Tahoe, fix input field on save.
*)
...
on new()
	...
	set decPreviewMarkup to script "core/dec-preview-markup"
	set decPreviewSidebar to script "core/dec-preview-sidebar"
	...
	script PreviewInstance
		...
		*)
		on setExportFilename(filename)
			...
			tell application "System Events" to tell process "Preview"
				-- set value of text field 1 of sheet 1 of front window to filename -- Pre-Tahoe
				-- first text field of splitter group 1 of sheet 1 of front window whose name is "Export As:"
				first text field of splitter group 1 of sheet 1 of front window whose name is "Save As:"
				set value of result to filename
			end tell
		...
		*)
		...
		on selectFormat(formatKey)
			...
			tell application "System Events" to tell process "Preview"
				...
				set formatDropDown to pop up button 2 of splitter group 1 of sheet 1 of front window
		...
		*)
		on triggerSave()
			...
			tell application "System Events" to tell process "Preview"
				click button "Save" of splitter group 1 of sheet 1 of front window
			end tell
		end triggerSave
		...
		on _new(pAppWindow)
			script PreviewTabInstance
				...
				on fullScreen()
					focus()
					-- activate application "Preview" -- prefer setting the frontmost property.
					tell application "System Events" to tell process "Preview"
						set frontmost to true
						try
	...
	decPreviewSidebar's decorate(result)
end new
